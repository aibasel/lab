...
class DownwardExperiment(Experiment):
    ...
    def compare_revisions(self, rev, base_rev=None, opt_or_sat='opt', configs=None,
        ...
        """
        Convenience method for setting up a comparison between a feature branch
        and its ancestor revision in the main branch or between two arbitrary
        Fast Downward revisions.

        Both revisions are tested on standard benchmark sets for the most
        important configurations by default. Reports that allow a before-after
        comparison are automatically added.

        *rev* determines the new revision to test.

        If *base_rev* is None (default), the latest revision on the default
        branch that is an ancestor of *rev* will be used.

        The value of *opt_or_sat* can be either 'opt' (default) or 'sat'.
        The automatically added standard sets of configurations and benchmark
        tasks are adapted to this choice. This can be overriden with the
        *suite* and *configs* parameters. If given, *suite* must be a string or
        a list of strings and *configs* must be a list of
        (nickname, configuration) pairs.

        *attributes* and *plot_attributes* are lists of attributes that will
        be part of the HTML and scatter plot reports. By default sets of
        standard attributes will be used for both.

        The automatically added HTML report is written to the path *outfile*,
        which can be absolute or relative to the evaluation directory. ::

            exp.compare_revisions('issue123')
            exp.compare_revisions('issue123',
                                  configs=[('lmcut', ['--search', 'astar(lmcut())'])])
            exp.compare_revisions(3310, 3300, optimal=False, suite='gripper')

        """
        if not base_rev:
            base_rev = checkouts.greatest_common_ancestor(self.repo, rev, 'default')
        self.combinations = checkouts.get_combinations(self.repo, rev, base_rev)
        if opt_or_sat == 'opt':
            if configs is None:
                configs = config_suite_optimal(
                    use_core_configs=True,
                    use_ipc_configs=True,
                    use_extended_configs=False).items()
            if suite is None:
                suite = suites.suite_optimal_with_ipc11()
        elif opt_or_sat == 'sat':
            if configs is None:
                configs = config_suite_satisficing(
                    use_core_configs=True,
                    use_ipc_configs=True,
                    use_extended_configs=False).items()
            if suite is None:
            ...
            logging.critical('opt_or_sat must be either "opt" or "sat".')
        if attributes is None:
            attributes = compare.DEFAULT_ATTRIBUTES
        if plot_attributes is None:
            plot_attributes = scatter.DEFAULT_PLOT_ATTRIBUTES
        self.add_suite(suite)
        for nick, config in configs:
            self.add_config(nick, config)

        self.add_report(
            compare.CompareRevisionsReport(base_rev, rev, attributes=attributes),
            outfile=outfile)
