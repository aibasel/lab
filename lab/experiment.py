...
class Experiment(_Buildable):
    ...
    def build(self, overwrite=False, only_main_script=False, no_main_script=False):
        ...
        if not no_main_script:
            ...
            # We only create the exp dir in the first part.
            # Overwrite exsiting files, but do not delete the whole directory.
            if os.path.isdir(self.path):
                runs_exist = any(path.startswith('runs') for path in os.listdir(self.path))
                if runs_exist:
                    logging.warning('The directory "%s" already contains run directories.' % self.path)
            else:
